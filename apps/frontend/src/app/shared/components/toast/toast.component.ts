    ...
    <div class="fixed top-4 right-4 z-50 flex flex-col gap-2">
      @for (toast of toastService.toasts(); track toast.id) {
        <div class="max-w-sm w-full shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5 transition-all overflow-hidden"
             ...
             [class.bg-emerald-800]="toast.type === 'info'">
          <div class="p-4 flex-1">
            <div class="flex items-start">
              ...
              <div class="ml-4 flex-shrink-0 flex">
                <button (click)="toastService.remove(toast.id)" class="bg-transparent rounded-md inline-flex text-slate-600 hover:text-slate-800 focus:outline-none">
